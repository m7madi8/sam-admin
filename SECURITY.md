...
- **تسجيل الدخول**: تُستخدم ملفات منفصلة:
  - `admin-auth.example.js`: نموذج فقط (يُرفع إلى Git).
  - `admin-auth.js`: يحتوي اسم المستخدم وكلمة المرور (مُستثنى من Git via `.gitignore`).
- إنشاء `admin-auth.js` من النموذج ووضع username و password.
