...
ملف **admin-auth.js** مضاف في `.gitignore` ولا يُرفع مع المشروع إلى Git أو الاستضافة. لذلك بعد رفع الموقع على الاستضافة **لن يوجد** الملف على السيرفر وستظهر رسالة "لم يُحمّل ملف admin-auth.js".

**ما يجب فعله بعد كل نشر:**

1. على جهازك المحلي، افتح الملف `assets/js/admin-auth.js` وانسخ محتواه كاملاً.
2. على الاستضافة (cPanel → مدير الملفات، أو FTP، أو لوحة الاستضافة):
   - انتقل إلى مجلد الموقع ثم إلى `assets/js/`
   - أنشئ ملفاً جديداً باسم **admin-auth.js**
   - الصق المحتوى الذي نسخته واحفظ الملف.
3. حدّث صفحة لوحة التحكم (f1.html) على الموقع وجرّب تسجيل الدخول.

بدون إنشاء هذا الملف على السيرفر، لوحة التحكم لن تعمل على الاستضافة (بينما تعمل محلياً لأن الملف موجود عندك).
